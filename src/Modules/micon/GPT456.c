...
void GPT456_init(void)
{
	...
	GPT5.GTUDC.BIT.UDF = 1;			// カウント方向強制設定する
	{
		GPT5.GTUDC.BIT.UD = 1;		// アップカウント
	}
	GPT5.GTUDC.BIT.UDF = 0;			// カウント方向強制設定解除
	...
	MPC.PWPR.BIT.PFSWE = 1;			// PFSレジスタ書き込み許可
	{
		...
		MPC.P94PFS.BIT.PSEL = 0x6;	// GTIOC5Aを選択
	}
	...
	GPT5.GTONCR.BIT.OAE = 0;
	GPT6.GTONCR.BIT.OAE = 0;

	// カウント開始
	GPTB.GTSTR.BIT.CST4 = 1;
	GPTB.GTSTR.BIT.CST5 = 1;
...
void Servo1_set(_UWORD fq, float duty)
{
	...
	GPT5.GTPR = cycle;
	GPT5.GTCCRA = (_UWORD)(cycle * duty);
}

void Servo1_On(void)
{
	GPT5.GTONCR.BIT.OAE = 1;
}
